    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # --- 1. 使用与训练时完全一致的硬编码 `::2` 来计算各部分 token 数量 ---
                
                # 目标视频 token (带时间戳)
                # 注意: t 是一个标量张量, 需要 reshape 并 expand 到 batch size
                t_reshaped = t.to(device=device, dtype=transformer_dtype).view(1, 1, 1, 1).expand(B, 1, 1, 1)
                # 创建一个与 latent 形状相同的 ones 张量作为 mask
                mask_grid = torch.ones((B, 1, F, H, W), device=device, dtype=transformer_dtype)
                
                # 使用 `::2` 对空间维度进行下采样来计算 token 数，与训练代码保持一致
                kkk = (t_reshaped * mask_grid[:, 0, :, ::2, ::2]).flatten(1)
                
                t_embed_parts = [kkk]

                # 条件视频 token (时间戳为0)
                if attention_kwargs.get("encoder_contion_states", None) is not None:
                    zeros_cond = torch.zeros_like(kkk)
                    t_embed_parts.append(zeros_cond)
                
                # 首帧 token (时间戳为0, 无时间维度)
                if attention_kwargs.get("encoder_first_states", None) is not None:
                    # 直接从 mask_grid 切片来确定形状，确保维度一致
                    zeros_first = torch.zeros_like(mask_grid[:, 0, 0, ::2, ::2].flatten(1))
                    t_embed_parts.append(zeros_first)

                # --- 2. 拼接成最终的 t_embed ---
                t_embed = torch.cat(t_embed_parts, dim=-1)

                # # --- 3. 【关键调试信息】---
                # # 我们只在第一次迭代时打印，避免刷屏
                # if i == 0:
                #     print("-" * 50)
                #     print(f"[DEBUG] Pipeline t_embed calculation at step {i}:")
                #     print(f"  - Latent shape (B,C,F,H,W): {latent_model_input.shape}")
                #     print(f"  - Target tokens (kkk) shape: {kkk.shape}")
                #     if "zeros_cond" in locals():
                #         print(f"  - Condition tokens (zeros_cond) shape: {zeros_cond.shape}")
                #     if "zeros_first" in locals():
                #         print(f"  - First-frame tokens (zeros_first) shape: {zeros_first.shape}")
                #     print(f"  - Final t_embed shape: {t_embed.shape}")
                #     print(f"  - Expected length from error: 1729. Calculated length: {t_embed.shape[1]}")
                #     print(f"  - This length MUST match the model's internal token count.")
                #     print("-" * 50)
