...
class WanTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def forward(
        ...
        if encoder_contion_states is not None:
            ...
            # breakpoint()
            encoder_contion_states = self.patch_embedding_extra(encoder_contion_states) # 用可学习的权重
        ...
        if rotary_emb2 is not None:
            ...
            if rotary_emb3 is not None: # 1转3
                # breakpoint()
                # 限制第一视角（输入）
                attn_mask[:, :, rotary_emb[0].shape[-2]: rotary_emb[0].shape[-2] + rotary_emb2[0].shape[-2], :rotary_emb[0].shape[-2]] = False # 第1视角 token 不能看其他部分
                attn_mask[:, :, rotary_emb[0].shape[-2]: rotary_emb[0].shape[-2] + rotary_emb2[0].shape[-2], -rotary_emb3[0].shape[-2]:] = False  # 其他部分不能看第1视角 token
                # 限制参考图片（输入）
                attn_mask[:, :, -rotary_emb3[0].shape[-2]:, :rotary_emb[0].shape[-2] + rotary_emb2[0].shape[-2]] = False  # 参考图像 token 不能看其他部分
            else: # 3转1
                # 限制第三视角（输入）
                # breakpoint()
        ...
        # breakpoint()
        # hidden_states.shape ---> torch.Size([2, 65520, 64])
        # -------------------------- additional ---------------------------
        if encoder_contion_states is not None:
            hidden_states = hidden_states[:, : rotary_emb[0].shape[-2]]
            # if encoder_first_states is not None: # 1转3，把3给解出来
            #     hidden_states = hidden_states[:, encoder_contion_states.shape[-2]:-encoder_first_states.shape[-2]]
            # else: # 3转1，把1给解出来
            #     hidden_states = hidden_states[:, :hidden_states.shape[1] // 2] # 把前一半取出来
