...
class WanVideoUnit_VACE(PipelineUnit):
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            if pipe.dit.in_dim == 48:
                # 针对 Wan2.2 5B 的逻辑: inactive(48) + reactive(48) = 96 个通道。
                # 这与 vace_in_dim=96 完美匹配，因此我们直接丢弃 mask latents。
                vace_context = vace_video_latents
            else:
                # 针对 Wan 1.3B/14B 的逻辑: inactive(16) + reactive(16) + mask(64) = 96 个通道。
                vace_context = torch.concat((vace_video_latents, vace_mask_latents), dim=1)
