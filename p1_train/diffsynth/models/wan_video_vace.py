...
class VaceWanModelDictConverter:
    ...
    def from_civitai(self, state_dict):
        # 改成 "vace" in name，容错带有 pipe.vace. 前缀的情况
        state_dict_ = {name: param for name, param in state_dict.items() if "vace" in name}
        
        # --- 自动识别我们刚才生成的 5B VACE 模型 ---
        is_5b = False
        for k, v in state_dict_.items():
            if 'vace_patch_embedding.weight' in k and v.shape[0] == 3072:
                is_5b = True
                break
        ...
        # ----------------------------------------------
        elif hash_state_dict_keys(state_dict_) == '3b2726384e4f64837bdf216eea3f310d': # vace 14B
            config = {
                "vace_layers": (0, 5, 10, 15, 20, 25, 30, 35),
                "vace_in_dim": 96,
                "patch_size": (1, 2, 2),
