...
#         # 同样改成 "vace" not in name，防止前缀导致过滤不干净从而污染 DiT 的 Hash 计算
#         state_dict = {name: param for name, param in state_dict.items() if "vace" not in name}
...
class WanModelStateDictConverter:
    ...
    def from_diffusers(self, state_dict):
        ...
        # --- 暴力特征识别：只要包含 5B 的核心维度参数，直接判定为 5B ---
        if 'patch_embedding.weight' in state_dict_ and state_dict_['patch_embedding.weight'].shape == (3072, 48, 1, 2, 2):
            config = {
                "has_image_input": False, "patch_size": [1, 2, 2], "in_dim": 48, "dim": 3072,
                "ffn_dim": 14336, "freq_dim": 256, "text_dim": 4096, "out_dim": 48,
                "num_heads": 24, "num_layers": 30, "eps": 1e-6, "seperated_timestep": True,
                "require_clip_embedding": False, "require_vae_embedding": False, "fuse_vae_embedding_in_latents": True,
            }
            return state_dict_, config
    ...
    def from_civitai(self, state_dict):
        ...
        # --- 同样加入暴力特征识别，防止某些原生权重也被误判 ---
        if 'patch_embedding.weight' in state_dict and state_dict['patch_embedding.weight'].shape == (3072, 48, 1, 2, 2):
            config = {
                "has_image_input": False, "patch_size": [1, 2, 2], "in_dim": 48, "dim": 3072,
                "ffn_dim": 14336, "freq_dim": 256, "text_dim": 4096, "out_dim": 48,
                "num_heads": 24, "num_layers": 30, "eps": 1e-6, "seperated_timestep": True,
        ...
        # 下面的旧逻辑保持不变，为了排版美观这里省略，直接用原来的即可...
        if hash_state_dict_keys(state_dict) == "9269f8db9040a9d860eaca435be61814":
            config = {
                "has_image_input": False,
                "patch_size": [1, 2, 2],
